...
static void goto_used(const Arg *arg);
...
/* CUSTOM */
static void findit(const Arg *arg );
static void first_unused(const Arg *arg);
...
void
first_unused(const Arg *arg){
  Client *c;
  unsigned int i, used_tags=0;
  Arg shiftit;
...
findit(const Arg *arg){
  ...
  if (fork() ==0){
    ...
    fprintf(stderr, "dwm: execvp %s", ((char **)new)[0]);
    perror(" failed");
    exit(EXIT_SUCCESS);
  }
}
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - sw - x) > bh) {
      ...
      if (strcmp(m->sel->name,"st")==0){
        strcpy(m->sel->name,"Terminal Emulator");
      }
